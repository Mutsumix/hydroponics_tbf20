...
使用するのは、香港のエスプレッソマシンメーカーDecentが製造する電子はかりです。この会社の特徴的な点は、製品の通信仕様をGitHubで公開し、サードパーティからの接続を歓迎していることです。BLEのサービスUUID、キャラクタリスティック、データフォーマットがすべてドキュメント化されており、3Dモデルまで公開する徹底ぶりです。

製品を開封してもマニュアル類は一切入っていません。ネット上で公開されている仕様書がすべて──ファームウェアのアップデートが前提の、現代的な製品らしい割り切りです。
...
アプリ側のコードでは、Androidの@<tt>{BluetoothLeScanner}を使い、デバイス名「Decent Scale」でフィルタリングしたスキャンを実行します。
...
3つのサービスが発見されました。ここで重要なのは@<tt>{FFF0}サービスです。この中に2つのキャラクタリスティックがあります。
...
ここで登場する@<b>{CCCD（Client Characteristic Configuration Descriptor）}は、Notifyを有効にするための特別なディスクリプタ（UUID: 2902）です。セントラルがこのディスクリプタに@<tt>{0x0001}を書き込むことで、「このキャラクタリスティックの変化を通知してほしい」とペリフェラルに伝えます。
...
@<b>{XORチェックサム}は、データが通信途中で壊れていないかを検証する仕組みです。バイト0から5までの値を順にXOR演算し、その結果がバイト6と一致すればデータは正常です。一致しなければ通信エラーが発生しているため、そのデータは破棄します。
...
開発者がSPPやRFCOMMを直接意識する必要がないのは、SDKが設計として意図した結果です。産業用機器のメーカーがSDKを提供する大きな理由がここにあります。Bluetooth Classicのペアリング、チャンネルの確立、エラーハンドリングといった低レベルの処理を開発者に任せると、実装の品質がバラつき、サポートコストが増大するためです。
...
注目すべきは、印刷のたびに接続と切断を行っている点です。常時接続を維持する方法もありますが、本書のアプリでは以下の理由から逐次接続方式を採用しています。
...
なぜAndroidから直接BLEで書き込まないのか。それは、電子ペーパータグへの書き込みにOpenEPaperLinkという特定のファームウェアプロトコルが必要で、このプロトコルを実装したAndroid向けのライブラリが存在しないためです。OpenEPaperLinkはESP32向けのファームウェアとして提供されており、ESP32がアクセスポイント（AP）として動作し、Wi-Fi経由でHTTPリクエストを受け付けます。
...
OkHttpを使ったmultipart/form-data形式のPOSTリクエストです。電子ペーパータグのMACアドレスと、JPEG画像データを送信しています。ESP32がこのリクエストを受け取ると、内部でBLEを使って電子ペーパータグに画像データを書き込みます。
...
デバイス	通信方式	通信の可視性	特徴
---------------------------------------------------------
電子はかり	BLE（GATT）	高い	Notify/Writeを直接操作。プロトコルを解読可能
モバイルプリンター	Bluetooth Classic（SPP）	低い	SDKが通信を隠蔽。印刷ロジックに集中可能
電子ペーパー	HTTP（ESP32経由）	なし（BLE部分）	中継機にBluetooth通信を委譲
//}

電子はかりでは、BLEのスキャン→GATT接続→サービス発見→Notifyサブスクライブ→データ受信→Writeコマンド送信という一連の流れを、ログの1行1行で追うことができました。第2章で理論として学んだGATT通信の仕組みが、実際のバイト列として目の前に現れる体験は、仕様書を読むだけでは得られないものです。

モバイルプリンターでは、SDKが通信の複雑さを隠蔽することで開発者体験を向上させている実例を見ました。Bluetooth ClassicのSPPが裏側で動いていることを知りつつも、開発者はそれを意識せずに印刷機能を実装できます。
