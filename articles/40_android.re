...
逆に、これまでAndroidアプリを作ったことがない方、これからAndroidアプリを作りたいという方は、この章を手を動かしながら読むことをおすすめします。
...
//footnote[gradle][Gradleについて解説すると、Android開発で標準的に使われるビルドツールです。ソースコードのコンパイル、ライブラリの依存関係の解決、APK（インストール用ファイル）の生成などを自動で行います。Android Studio上では意識せずとも裏で動いていますが、ライブラリの追加時などに@<tt>{build.gradle.kts}ファイルを編集する場面が出てきます。]
...
//image[emulator][エミュレータで「Hello Android!」が表示されている画面][scale=0.35]
...
次章からは、これらの基礎知識を前提に、AIを活用してアプリの実装を進めていきます。